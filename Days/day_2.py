...
and position 10 contains 40. Add these numbers together to get 70. Then, store this value at the output position; here,
...
Your puzzle answer was 6627023.
...
are called the instruction's parameters. For example, in the instruction 1,2,3,4, 1 is the opcode; 2, 3, and 4 are the parameters.
...
Once the program has halted, its output is available at address 0, also just like before. Each time you try a pair of inputs,
make sure you first reset the computer's memory to the values in the program (your puzzle input) - in other words, don't reuse memory from a previous attempt.

Find the input noun and verb that cause the program to produce the output 19690720. What is 100 * noun + verb? (For example, if noun=12 and verb=2, the answer would be 1202.)

Your puzzle answer was 4019.

Both parts of this puzzle are complete! They provide two gold stars: **"""

l = open('data_day_2.txt')
l = l.read().split(',')
l = [int(x) for x in l]
...
#
# # solution A
# # def store_to(l, idx, var):
# #     l[idx] = var
# #
# # [store_to(l, l[i + 3], l[l[i + 1]] + l[l[i + 2]]) if l[i] == 1 else store_to(l, l[i + 3], l[l[i + 1]] * l[l[i + 2]]) if
# # l[i] == 2 else None for i in range(0, len(l), 4)]
