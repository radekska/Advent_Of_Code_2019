...
Your puzzle answer was 3372756.
...
that fuel also requires fuel, and that fuel requires fuel, and so on.
...
Your puzzle answer was 5056279.

Both parts of this puzzle are complete! They provide two gold stars: **"""

mass_list = open("data_day_1.txt")
mass_list = mass_list.read()
mass_list = mass_list.replace("\n", ',')
mass_list = mass_list.split(',')
mass_list = [int((int(x) / 3)) - 2 for x in mass_list]
print(mass_list)


def fuel_of_fuel(fuel, sum=0):
    if int(fuel / 3) - 2 <= 0:
        return sum + fuel
    else:
        sum += fuel
        fuel = int(fuel / 3) - 2
        return fuel_of_fuel(fuel, sum)


mass_list_rec = map(fuel_of_fuel, mass_list)
mass_list_rec = list(mass_list_rec)
print(sum(mass_list_rec))
